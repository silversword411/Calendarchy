...
- **Motion & feel**: nothing snaps. View switches, the event editor opening/closing, and the sidebar collapsing (this section's hamburger toggle) all animate through libadwaita's own animation primitives (`Adw.TimedAnimation`, `Adw.SpringAnimation`, `Gtk.Revealer`) rather than hard cuts or a hand-rolled easing curve, so motion stays consistent with the rest of the Omarchy desktop instead of feeling bolted on. Every hoverable control — grid cells, event chips, sidebar calendar rows, the "⋮" per-calendar menu trigger, toolbar buttons — gets a subtle background-shade hover state from the inherited Adwaita stylesheet (§14), so pointer feedback shows up everywhere it's expected without one-off custom CSS. Custom widgets libadwaita doesn't provide out of the box — event chips/pills, mini-month grid cells, the view-switcher pill group — are styled to match Adwaita's existing button/row conventions rather than looking hand-rolled themselves.
