...
- **Dirty guard**: any dialog holding editable, not-yet-saved state tracks a dirty flag and guards its close path with a "Discard unsaved changes?" confirmation — closing with nothing changed just closes, no prompt. Reference implementation: `install_close_guard` (hooks `adw::Window`'s `close-request`) for dialogs built on `adw::Window`; the Settings window (`adw::Dialog`) guards the equivalent way via `close-attempt`/`can-close`/`force_close` in `install_settings_close_guard`, since `adw::Dialog` doesn't expose `close-request`.
- **Escape always closes**: bound via a `gtk4::ShortcutController` with `ShortcutScope::Global`, so it fires no matter which field currently has keyboard focus, and always routes through the guard above rather than bypassing it (a dialog's own Save action closes by destroying the window directly, which is the one intentional bypass — a successful save has nothing left to discard). Reference: `install_escape_to_close`.
...
New dialogs should copy one of the two reference implementations above rather than hand-rolling close/Escape handling again.
