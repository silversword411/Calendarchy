...
- Native GTK4 window with month / week / day / agenda / list views.
...
- **CalDAV, Outlook, or other providers.** Google Calendar only for account-based sync. The account/sync layer is written so this isn't foreclosed later, but it's not in scope now. (One narrow exception: read-only ICS-URL calendar subscriptions, §12 — not a provider integration, since it involves no account, no OAuth, and no write support.)
- **True real-time push.** See §9 — this would require a hosted relay service, which is explicitly deferred (§21).
...
- **Arch Linux**, distributed as an AUR package (§17).
...
- **`calendars`** — `id`, `account_id` (FK, **nullable**), `google_calendar_id` (nullable), `source_url` (nullable — set only for ICS-URL calendars, §12), `display_name`, `color`, `is_visible` (mirrors Google's `calendarList.selected` field, §10), `access_role` (owner/writer/reader — controls whether edits are allowed; always `reader` for ICS-URL calendars). Owned by the Calendar service; a future Contacts service would add its own `contacts` table following the same per-service-ownership pattern rather than growing this table. `account_id`/`google_calendar_id` are only null for the ICS-URL case (§12) — every Google-backed calendar still has both.
- **`events`** — `id`, `calendar_id` (FK), `google_event_id`, `title`, `description`, `location`, `start`, `end`, `all_day`, `recurrence_rule`, `status`, `self_response_status` (accepted/declined/tentative/needsAction — the signed-in account's own RSVP, read off the event's `attendees[]` array; drives "Show declined events" in §13), `etag`, `updated_at`.
...
- **`app_settings`** — `key`, `value` (JSON), `updated_at`. A single key/value table backing the Preferences window (§13) — language/region overrides, time zone display settings, event/view/notification defaults, etc. — so preferences sync with the rest of the app's state instead of living in a separate config file.
...
- **Left sidebar**: collapsible via a hamburger icon in the headerbar (hides/shows the whole pane, independent of any individual calendar's visibility). Top to bottom: a **Create** button opening the event editor below — its dropdown arrow mirrors Google's quick-create menu, but only the "Event" entry does anything in v1 (Task is greyed out pending the future Tasks service, §6; Appointment schedule and Out of office are Workspace-booking features and out of scope, see below); a **mini-month date navigator** (click a date to jump the main view there, arrows to page months, independent of whichever main view is active); the **World clock** module (§13) directly underneath, when enabled; a **Search for people** box — an ad hoc, non-persistent overlay that looks up a colleague by email and temporarily shows their availability in the main grid without adding anything to a calendar list (contrast with §12's Subscribe to calendar, which does persist); and two collapsible checklists, **My calendars** (this account's own primary/secondary calendars) and **Other calendars** (anything added via §12's Subscribe/Browse/From-URL flows), each row wired to the per-calendar sidebar menu below. **Booking pages** — Google Workspace's appointment-scheduling links — is out of scope; it's a scheduling product layered on top of Calendar, not a calendar-viewing/editing capability (§2).
- **Views**: Month, Week, Day, Year, and Agenda (list) — matching the core Google Calendar PWA view set — plus a **5-day work week** view (Mon–Fri, Google's "5 days" option) and **List** — a flat, Outlook-style table view, distinct from Agenda's day-grouped card layout (detailed below). Year is the cheapest of the seven to implement: a grid of 12 mini-months for navigation, with no per-event rendering needed.
- **List view**: a flat, spreadsheet-style table of events — one row per event — modeled on classic (Windows legacy) Microsoft Outlook's Calendar List/Table view, in contrast to Agenda's day-grouped cards above. **v1** (lands in Phase 5, §20, alongside the other remaining views, since write support already exists by then per Phase 3): default columns Subject, Start, End, Duration, Location, Calendar/Account, All Day, Recurrence indicator, and Status/Show-Time-As, each row colored by its calendar's `color` (§8 — Calendarchy has no separate per-event category system the way Outlook does, so "category color" maps to calendar color); click a column header to sort ascending/descending with a direction indicator, and a modifier-click (or an "Arrange By" menu) layers up to 4 sort levels; drag-to-reorder columns, drag-to-resize columns, and a "Best Fit" auto-size action from the header's right-click menu; a **Field Chooser** (right-click a header, or a toolbar button) to add/remove columns from the fields that exist in §8's `events` schema (title, location, start, end, computed duration, all_day, recurrence_rule presence, status, self_response_status) — fields Outlook shows but Calendarchy's schema doesn't model yet, like Organizer/Attendees, are explicitly not in the v1 chooser; and **in-cell editing** — single-click Subject or Location to edit inline, double-click/Enter still opens the full Event editor above. **Stretch** (picked up opportunistically post-Phase-5, the same treatment §20 already gives the Quickshell panel widget, §14b): a **Group By box** to drag a column header above the grid and collapse rows into nested, multi-level collapsible groups (by date, calendar, status); **conditional formatting** — user-defined rules coloring a row's text/background by field value, beyond the baseline calendar-color coding; **saved custom views** — name and persist a column set + sort + grouping + filter combination (stored via §8's `app_settings` table) and switch between them from a view picker, mirroring Outlook's "Change View"/"Manage Views"; and a List-only **filter bar** (by calendar, status, or date range).
- **Event editor**: title, time (with all-day toggle), location, description, calendar/account picker, and reminder lead time, rendered as a dockable panel rather than a fixed modal dialog. It has three layouts — **floating** (a card that floats over the calendar grid), **bottom-docked** (spanning the full width of the main content area), and **right-docked** (mirroring the left sidebar) — and is non-modal in all three: the calendar underneath/around it stays fully interactive. Dragging the panel by its header repositions it while floating; crossing near the content area's bottom or right edge snaps it into that dock, and dragging a docked panel's header away from its edge pulls it back out to floating. A docked panel can be resized by dragging its free edge (a `Paned` handle, the same mechanism as the sidebar's own resize). The chosen layout, plus each dock's size and the floating position, are remembered (`app_settings`, §8) and restored the next time the editor opens. Doubles as the Create Event dialog the same way it always has.
...
- **Keyboard navigation**: arrow keys / vim-style `hjkl` to move between days/weeks, `n` for new event, `/` to search, and Google's own single-letter view shortcuts reused as-is — `D`/`W`/`M`/`Y`/`A`/`X` for Day/Week/Month/Year/Agenda/5-day work week — plus `L` for the Outlook-style List view, which has no Google Calendar equivalent, rather than inventing Calendarchy-specific bindings for the rest.
- **Motion & feel**: nothing snaps, with one deliberate exception — dragging the event editor panel (this section's redesigned dockable editor) snaps it into a bottom/right dock once it crosses a proximity threshold near that edge, and un-docks the same way when dragged clear of it. Everywhere else: view switches, the editor's open/close, and the sidebar collapsing (this section's hamburger toggle) all animate through libadwaita's own animation primitives (`Adw.TimedAnimation`, `Adw.SpringAnimation`, `Gtk.Revealer`) rather than hard cuts or a hand-rolled easing curve, so motion stays consistent with the rest of the Omarchy desktop instead of feeling bolted on. Every hoverable control — grid cells, event chips, sidebar calendar rows, the "⋮" per-calendar menu trigger, toolbar buttons — gets a subtle background-shade hover state from the inherited Adwaita stylesheet (§15), so pointer feedback shows up everywhere it's expected without one-off custom CSS. Custom widgets libadwaita doesn't provide out of the box — event chips/pills, mini-month grid cells, the view-switcher pill group — are styled to match Adwaita's existing button/row conventions rather than looking hand-rolled themselves. The event editor's own close/discard behavior follows §11's dialog conventions.

## 11. Dialog & Popup Conventions

Every modal dialog and popover in the app — the event editor, Settings, and anything added later — follows the same keyboard/close conventions, so this doesn't have to be rediscovered (and re-fixed) per dialog:

- **Dirty guard**: any dialog holding editable, not-yet-saved state tracks a dirty flag and guards its close path with a "Discard unsaved changes?" confirmation — closing with nothing changed just closes, no prompt. Two reference implementations, chosen by what the dialog is built on: `install_close_guard` (hooks `adw::Window`'s `close-request`) for dialogs built on their own `adw::Window`; the Settings window (`adw::Dialog`) guards the equivalent way via `close-attempt`/`can-close`/`force_close` in `install_settings_close_guard`, since `adw::Dialog` doesn't expose `close-request`. The event editor (§10's redesigned dockable panel) is neither — it's a plain embedded widget with no window of its own to hang a `close-request` signal off — so it uses a third pattern instead: `confirm_close` runs the same "Discard unsaved changes?" `AlertDialog` directly, called from each close trigger rather than installed as a signal handler.
- **Escape always closes**: bound via a `gtk4::ShortcutController` with `ShortcutScope::Global`, so it fires no matter which field currently has keyboard focus, and always routes through the guard above rather than bypassing it (a dialog's own Save action closes by destroying/detaching the dialog directly, which is the one intentional bypass — a successful save has nothing left to discard). Reference: `install_escape_to_close` for `adw::Window`-based dialogs; `install_panel_escape_to_close` for the event editor's embedded panel, attaching the same `Global`-scope controller to the panel's own root widget instead of a window (`Global` scope resolves at the nearest `Root` ancestor regardless of which widget it's attached to, so this works the same way once the panel is parented anywhere in the main window's tree).
- **Two-stage Escape in a dialog with a search/filter field**: first press clears the field; a second press (or a press while it's already empty) closes the dialog through the same guarded path. Implement both stages in one handler owning the whole decision — not a search field's own clear-on-Escape behavior running alongside a separate global close-on-Escape handler, which races two independent handlers on the same key. Reference: `install_escape_to_close_dialog`, used by the Settings window's sidebar search.
- **Lightweight popovers are exempt**: a `gtk4::Popover` with no persisted unsaved state (a menu, a color picker, an insert-link prompt, the event quick-view popover) relies on GTK4's own default Escape-dismisses-popover behavior. Don't add guard machinery here — there's nothing to lose by dismissing it.
- **Simple confirmations set a safe `close_response`**: an `adw::AlertDialog` posing a yes/no question (discard changes, delete an event, clear the cache) calls `set_close_response(...)` pointed at the non-destructive choice, so Escape and a window-manager close both land on "cancel" rather than the destructive option, without any custom Escape wiring at all.

New dialogs should copy one of the three reference implementations above (window-based, `adw::Dialog`-based, or embedded-panel-based) rather than hand-rolling close/Escape handling again.
...
- **Language and region** — an in-app override, independent of the system locale Calendarchy otherwise follows (the same one it reads `$LANG`/`LC_TIME` from for GTK4 theme inheritance, §15). **Language** and **Country** are stored pickers with no visible effect yet — Calendarchy has no translation catalog, and Country isn't consumed anywhere until §12's region-keyed holiday-calendar shortlist exists — kept "Automatic (system)" by default. **Date format** (month/day/year order) and **Time format** (12h/24h) apply immediately app-wide (event editor date pickers, event popover, event chips) and also default to "Automatic (system)", which falls back to a locale heuristic rather than a fixed choice.
...
- **Event settings** — defaults applied by the event editor (§10) when creating a new event: default duration (e.g. 30/60 min), default calendar (which account+calendar a quick-add lands in), default reminder lead time (feeds §14's Notification Scheduler unless overridden per-event), guest permissions (modify event / invite others / see guest list), whether to auto-add Google Meet conferencing, and how received invitations are handled ("add invitations to my calendar" — all/known senders only/none — and whether editors of this calendar can see pending invitations). No "speedy meetings" auto-shortening or other Workspace-side event defaults — those are computed server-side by Google's own client (see Google Workspace smart features, below).
- **Notification settings** — global defaults layered under the per-event lead time in the editor: desktop-notification on/off per calendar (mute a noisy calendar without disabling its sync), a notification sound toggle, the default lead time used when an event doesn't specify its own, how long before the event a snoozed notification resurfaces ("show snoozed notifications"), and an option to only notify for events RSVP'd "Yes" or "Maybe" — mirroring the Google Calendar PWA's own notification settings panel. The delivery mechanism itself is §14.
- **View options** — default view on launch (any view from §10, including Year, the 5-day work week, and List), density (comfortable/compact row height), **Show weekends**, **Show declined events** (dimmed rather than hidden, driven by the new `self_response_status` field on `events`, §8), **Show week numbers**, **Display shorter events the same size as 30 minute events**, **Reduce the brightness of past events**, and **View calendars side by side in Day View** — mirroring the Google Calendar PWA's own View options checklist. **Show completed tasks** appears in the same list but greyed out — "coming soon," the same treatment §6 gives the Contacts/Notes/Tasks service toggles — until the Tasks service actually exists to supply completed-task data. **Start week on** here is a per-app override of the region default above, matching Google Calendar's own split between region-implied and explicitly-set start day. **Set custom view** picks the day count for the sidebar's custom N-day view (§10's fixed 5-day work week is this setting's default, not a separate control). **Alternate calendars** overlays a second date system (Chinese, Hebrew Lunar, Islamic Lunar) under each grid date, purely a display overlay — unlike the Google Workspace smart features below, it needs no extra scopes and isn't itself a non-goal, just not yet rendered.
- **Google Workspace smart features** — explicitly **not implemented** (§2). Google's smart features are computed by Google's own backend against broader Workspace scopes (Gmail, Drive) this app deliberately doesn't request, kept as narrow as each service's actual usage needs (§16), and aren't exposed through the Calendar API a pure-calendar client consumes. Tracked as a non-goal rather than a setting with no effect.
- **Keyboard shortcuts** — a read-only `Gtk::ShortcutsWindow` cheat sheet (opened with `?` or from Preferences) listing the bindings already defined in §10, plus every dialog's Escape-to-close behavior from §11. In-app rebinding is out of scope for v1; the shortcuts are fixed, not configurable.
...
## 14. Notification Panel Integration
...
- **Theming** — no custom CSS theme; GTK4/libadwaita picks up Omarchy's system theme (accent color, light/dark) automatically. Hover shading and motion (§10) ride the same inherited Adwaita stylesheet, so they follow the system theme too rather than needing separate light/dark handling.
...
- Semantic versioning starting at `0.1.0` through the MVP phases in §20.
...
5. **Polish & packaging** — Week/Day/Year/5-day-work-week/List views, view options (§13), search, keyboard nav completeness, AUR packaging, Hyprland/theming polish.

Quickshell panel widget integration (§14b) is intentionally not a numbered phase — it's picked up opportunistically once phase 4 is stable. Contacts/Notes/Tasks services (§6) are intentionally not numbered phases either — they're future work enabled by phase 0, not committed to a timeline here. List view's stretch tier — Group By box, conditional formatting, saved custom views, and its per-view filter bar (§10) — is likewise deliberately excluded from phase 5's baseline and picked up opportunistically afterward, the same treatment as the Quickshell panel widget.
...
- **Quickshell panel widget** (§14b): scope this once the freedesktop-notification path (§14a) is shipped and the Quickshell plugin/IPC API has been evaluated hands-on.
